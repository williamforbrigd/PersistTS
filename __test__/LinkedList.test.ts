...
describe('LinkedList', () => {
    ...
    test('addAllAtIndex()', () => {
        const result = list.addAll([1, 2, 3]);
        const result2 = result.addAll([4, 5], 1);
    ...
    test('removeItem()', () => {
        const result = LinkedList.of("Hello", "World");
        const removed = result.removeItem("Hello");
    ...
    test('pop() one element is size - 1', () => {
        const lst = LinkedList.of(1, 2, 3)
        const result = lst.pop();
        expect(result).toEqual(LinkedList.of(1, 2));
    })
